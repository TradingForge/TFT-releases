...
- **Source code only (`.mq4`), not compiled `.ex4`.** The Terminal compiles the original MQL4
  ...
  decompilation.
- **Binance stop-loss / take-profit are handled by the Terminal, not by the exchange.**
  S/L and T/P are watched locally and the position is closed by the Terminal when price
  reaches the level. They are **not** placed as native orders on Binance yet — so they
  only work while the Terminal is running and connected to the account. Exchange-side
  placement is coming in a future version (see *Coming soon*). (MetaTrader 4 accounts use
  the broker's own native S/L and T/P.)
