...
```mermaid
flowchart LR
    A["MQL EA script (.mq4)"] --> B["TFT Transpiler"]
    B --> C["C# EA script"]
    C --> D["TFT Runtime"]
    D --> E["Execution venue"]
```

Original MQL on one side, transpiled C# on the other — the C# is the code TFT actually compiles and runs:
...
```mermaid
flowchart LR
    subgraph SRC["Strategy source"]
        S1["MT4 script"]
        S2["MT5 script (planned)"]
        S3["TradingView (planned)"]
    end
    subgraph TFT["TradingForge Terminal"]
        R["TFT Runtime"]
        T["TFT Transpiler"]
        R --> T
    end
    subgraph OUT["Brokers and exchanges"]
        B1["MT4"]
        B2["Binance USDT futures"]
        B3["OKX (planned)"]
        B4["cTrader (planned)"]
        B5["Interactive Brokers (planned)"]
        B6["FIX-protocol brokers (planned)"]
        B7["Proprietary-API brokers (planned)"]
        B8["DEX (planned)"]
        B9["Custom backends (planned)"]
    end
    S1 --> R
    S2 --> R
    S3 --> R
...
- **Binance stop-loss / take-profit are handled by the Terminal, not by the exchange.** S/L and T/P are
  ...
  **not** placed as native orders on Binance yet — so they only work while the Terminal is running and
  connected to the account. Exchange-side placement is coming in a future version (see *Coming soon*).
...
- Updates: the Terminal offers new versions itself (*Help → Check for updates*). Your accounts, settings
  and Expert Advisors are preserved.
